"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
var core_1 = require("@angular/core");
var http_1 = require("@angular/http");
var rxjs_1 = require("rxjs");
var globalVariables = require("../global-variable");
var AbsenceRequestService = (function () {
    // Resolve HTTP using the constructor
    function AbsenceRequestService(http) {
        this.http = http;
        // private instance variable to hold base url
        this.newRequestsUrl = globalVariables.apiHost + '/absence-request/student/new';
        this.acceptedRequestsUrl = globalVariables.apiHost + '/absence-request/student/accepted';
        this.acceptRequestUrl = globalVariables.apiHost + '/absence-request/student/accept';
    }
    AbsenceRequestService.prototype.getNewRequests = function () {
        return this.http.get(this.newRequestsUrl)
            .map(function (res) { return res.json(); })
            .catch(function (error) { return rxjs_1.Observable.throw(error || 'Server error'); });
    };
    AbsenceRequestService.prototype.getAcceptedRequests = function () {
        return this.http.get(this.acceptedRequestsUrl)
            .map(function (res) { return res.json(); })
            .catch(function (error) { return rxjs_1.Observable.throw(error || 'Server error'); });
    };
    AbsenceRequestService.prototype.acceptRequest = function (id) {
        var bodyString = { 'id': id };
        var headers = new http_1.Headers({ 'Content-Type': 'application/json' });
        var options = new http_1.RequestOptions({ headers: headers });
        return this.http.put(this.acceptRequestUrl, bodyString, options)
            .map(function (res) { return res; })
            .catch(function (error) { return rxjs_1.Observable.throw(error || 'Server error'); });
    };
    return AbsenceRequestService;
}());
AbsenceRequestService = __decorate([
    core_1.Injectable(),
    __metadata("design:paramtypes", [http_1.Http])
], AbsenceRequestService);
exports.AbsenceRequestService = AbsenceRequestService;
//# sourceMappingURL=absence-request.service.js.map